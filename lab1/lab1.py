...
same_id = IF(OR('male (?x)','female (?x)'),THEN('same_identity (?x) (?x)'));
sibling = IF(AND('parent (?p) (?x)','parent (?p) (?y)',NOT('same_identity (?x) (?y)')),THEN('sibling (?x) (?y)'));
brother = IF(AND('sibling (?x) (?y)','male (?x)'),THEN('brother (?x) (?y)'));
sister = IF( AND('sibling (?x) (?y)','female (?x)'),THEN('sister (?x) (?y)'));
mother = IF(AND('parent (?p) (?x)','female (?p)'),THEN('mother (?p) (?x)'));
father = IF(AND('parent (?p) (?x)','male (?p)'),THEN('father (?p) (?x)'));
daughter = IF(AND('parent (?p) (?x)','female (?x)'),THEN('daughter (?x) (?p)'));
son = IF(AND('parent (?p) (?x)','male (?x)'),THEN('son (?x) (?p)'));
...
grandchild = IF(AND('grandparent (?a) (?b)'),THEN('grandchild (?b) (?a)'));

family_rules.extend([same_id, sibling,brother,sister,mother,father,son,daughter,cousin,grandparent,grandchild]);
...
black_family_cousins = [ 
    x for x in
    forward_chain(family_rules, black_data, verbose=debug)
    if "cousin" in x ]
...
HOW_MANY_HOURS_THIS_PSET_TOOK = '2 while watching the premier league'
