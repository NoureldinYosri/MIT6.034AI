...
def sum_sum(expr1,expr2):
    e1 = [term for term in expr1];
    e2 = [term for term in expr2];
    if len(e1) == 1 and len(e2) == 1:
        ...
        if isinstance(u,int) and isinstance(v,int):
            ...
            return Product([u,v]);
    ret = Sum([Sum([Product([x,y]) for x in e1]) for y in e2]);
    ret = ret.flatten();
    res = [];
    coef = 0;
    for term in ret:
        if isinstance(term,int):
            ...
            tmp = [x for x in term];
            if len(tmp) != 1: res.append(term);
            else:
                x = tmp[0];
                if isinstance(x,int): coef += x;
                else: res.append(term);
        else:
            res.append(term);
    if coef != 0: res.append(coef);
    return Sum(res);

def sum_prod(expr1,expr2):
    ret = [multiply(e,expr2) for e in expr1];
    res = [];
    coef = 0;
    for term in ret:
        if isinstance(term, int):
...
def prod_prod(expr1,expr2):
    e1 = [term for term in expr1];
    e2 = [term for term in expr2];
    c1 = filter(lambda x:isinstance(x,int),e1);
    c2 = filter(lambda x:isinstance(x,int),e2);
    r1 = filter(lambda x: not isinstance(x, int), e1);
    r2 = filter(lambda x: not isinstance(x, int), e2);
    coef = 1;
    if len(c1) != 0: coef *= reduce(lambda a,b:a*b,c1);
    if len(c2) != 0: coef *= reduce(lambda a,b:a*b,c2);
    return Product([coef] + r1 + r2);
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    is1_sum = isinstance(expr1,Sum);
    is2_sum = isinstance(expr2,Sum);

    res = None;
    if is1_sum and is2_sum: res = sum_sum(expr1,expr2);
    elif is1_sum and (not is2_sum): res =  sum_prod(expr1,expr2);
    elif (not is1_sum) and is2_sum: res = sum_prod(expr2,expr1);
    else: res = prod_prod(expr1,expr2);
    #print expr1,expr2,"->",res;
    return res;

if __name__ == "__main__":
    #P = Product([Product([5]),Product([6])]).flatten().simplify();
    #S = S.simplify();
    #print P;
    a = Sum([5]);
    b = Product([5,6]);
